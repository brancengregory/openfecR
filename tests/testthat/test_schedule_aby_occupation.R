# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate

context("Test ScheduleAByOccupation")

model.instance <- ScheduleAByOccupation$new()

test_that("committee_id", {
  # tests for the property `committee_id` (character)
  #  A unique identifier assigned to each committee or filer registered with the FEC. In general committee id&#39;s begin with the letter C which is followed by eight digits. 

  # uncomment below to test the property 
  #expect_equal(model.instance$`committee_id`, "EXPECTED_RESULT")
})

test_that("count", {
  # tests for the property `count` (integer)
  #  Number of records making up the total. 

  # uncomment below to test the property 
  #expect_equal(model.instance$`count`, "EXPECTED_RESULT")
})

test_that("cycle", {
  # tests for the property `cycle` (integer)
  #  Filter records to only those that were applicable to a given two-year period.The cycle begins with an odd year and is named for its ending, even year. 

  # uncomment below to test the property 
  #expect_equal(model.instance$`cycle`, "EXPECTED_RESULT")
})

test_that("occupation", {
  # tests for the property `occupation` (character)
  # Occupation of contributor as reported on the committee&#39;s filing

  # uncomment below to test the property 
  #expect_equal(model.instance$`occupation`, "EXPECTED_RESULT")
})

test_that("total", {
  # tests for the property `total` (numeric)
  # Sum of transactions

  # uncomment below to test the property 
  #expect_equal(model.instance$`total`, "EXPECTED_RESULT")
})

